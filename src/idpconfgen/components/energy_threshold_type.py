...
calculates the energy as a single float for the whole conformer (performs sum of
pair contributions. `pairs` calculates energy for individual pairs. In the
current implementation, the energy threshold (-etbb and -etss) will be compared
to the `whole` calculation or to the individual `pairs`. In `pairs`, if a single
pair has energy above threshold the conformer is discarded. If you use `pairs`,
the minimum values for `-etbb` and `-etss` should not be bellow 2 or 3 because
pairs at 1-4 (three bonds apart) will always have a small positive energy
