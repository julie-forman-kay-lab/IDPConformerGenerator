...
IDPCalc Conformer Generator implements different parametrized search and construction approaches. Yet, a main idea sets a **common** ground: backbone (*psi*, *phi* and *omega*) angle sets must be provided in *fragments* of a predefined (user defined) minimum length and not individually for each residue.
...
The necessary software resources to download and prepare the databases from a *cullpdb* file are provided as separated interfaces developed for this project, see examples at the bottom of this requirements document. Some of these interfaces rely on third party libraries, such as Rosetta or DSSP.
...
Under this approach, conformers are built by using angles from protein fragments of unique secondary structure regions found in the database. The three different secondary structure types are: helices, sheets and loops. **Initially, and generally**, this approach focused only on **loop** regions, however, it can be abstracted out to include helices and sheets, if required.

In other words, given a target protein of 100 residues, unique secondary structure regions (by default *only* loops) are sequentially and randomly selected from the database and their set of angles are used to build the conformer backbone, fragment by fragment. Different regions are concatenated until the length of the target protein is fulfilled.

This is the simplest approach and does not consider residue identity; it only considers angles *consecutiveness* integrity, that is, angles used to build conformers are given as *fragments* of different lengths and NOT as isolated residue angles.
...
A conformer can be built by randomly selecting and combining angle *fragments* from the sequence-angle map obtained by the parametrized and curated search step.
...
A main command line interface will serve as entry point for the other subroutines. For example, `idpconfgen` is the current main entry point. Subroutines are passed as arguments:

```
$ idpconfgen pdbdl [options]
```

where `pdbdl` provides the interface to download PDBs from the *CULL* file.

**Other interfaces envisioned:**
* `pdbdl`, downloads PDB
* `ssext`, extracts secondary structure information
* `pdbsplit`, splits PDBs for idealization
* `idealize`, idealizes PDBs
* `angext`, extract omega, psi, phi angles
* `confbuild`, build the conformers

Other interfaces will be added as requested.

### Usage

The following describes an example for running the conformer builder:

```
idpconfgen confbuild \
