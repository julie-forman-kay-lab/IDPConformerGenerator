...
IDPCalc Conformer Generator implements different parametrized search and construction approaches. Yet, a main idea sets a **common** ground: backbone (*psi*, *phi* and *omega*) angle sets must be provided in *fragments* of a predefined (user defined) minimum size and not individually for each residue.
...
This is the simplest approach and does not consider residue identity; it only considers angles *consecutiveness* integrity, that is, angles used to build conformers are given as *fragments* of different sizes and NOT as isolated residue angles.
